            ...
            <el-card shadow="hover">
              <el-form label-width="60px">
                <el-form-item label="名称">
                  <el-input v-model="item.name" placeholder="请输入设施名称" />
                </el-form-item>
                <el-form-item label="描述">
                  <el-input v-model="item.description" type="textarea" :rows="2" placeholder="请输入描述" />
                </el-form-item>
                <el-form-item label="图片">
                  <el-input v-model="item.image" placeholder="请输入图片URL" />
                </el-form-item>
                <el-button type="danger" size="small" @click="removeItem(index)">删除</el-button>
              </el-form>
            </el-card>
          </div>
          <el-button type="primary" plain @click="addItem">+ 添加设施</el-button>
        </div>
      </el-form-item>
...
async function loadConfig() {
  try {
    const { data } = await http.get('/homepage/config/facilities')
    if (data.data?.config) {
...
async function handleSave() {
  saving.value = true
  try {
    await http.put('/homepage/config/facilities', { config: form.value })
